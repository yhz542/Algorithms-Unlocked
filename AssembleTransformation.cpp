...
vector<int> AssembleTransformation(vector<vector<int>> &, size_t, size_t);
int main()
{
	string X, Y;
	getline(cin, X);
	...
	vector<int> result = AssembleTransformation(op, m, n);
...
vector<int> AssembleTransformation(vector<vector<int>> &op, size_t i, size_t j)
{
	if (!i && !j)
		return vector<int>();
	else
	{
		vector<int> tem;
		if (op[i][j] == CC || op[i][j] == CR)
			tem = AssembleTransformation(op, i - 1, j - 1);
		else if (op[i][j] == CD)
			tem = AssembleTransformation(op, i - 1, j);
		else
			tem = AssembleTransformation(op, i, j - 1);
		tem.push_back(op[i][j]);
		return tem;
	}
}
